...
parser.add_argument('--animation', dest='animation',
                    action='store_true', default=False)
...
def sample_text(sess, args_text, translation, style=None):
    ...
    sequence = np.expand_dims(np.concatenate([sequence,
                                              np.zeros((1, len(translation)))]), axis=0)
    ...
    for s in range(1, 60 * sequence_len + 1):
        ...
        print('\r[{:5d}] sampling... {}'.format(s,
                                                'priming' if is_priming else 'synthesis'), end='')
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            phi_data, window_data, kappa_data, stroke_data, coords = sample_text(
                sess, args_text, translation, style)
            ...
            if args.info:
                ...
                for i in range(strokes.shape[0]):
                    gauss = mlab.bivariate_normal(x_grid, y_grid, mux=strokes[i, 0], muy=strokes[i, 1],
                                                  sigmax=strokes[i,
                                                                 2], sigmay=strokes[i, 3],
                                                  sigmaxy=0.)  # strokes[i, 4]
                    z_grid += gauss * \
                        np.power(strokes[i, 2] + strokes[i, 3],
                                 0.4) / (np.max(gauss) + epsilon)

                fig, ax = plt.subplots(2, 2)

                ax[0, 0].imshow(z_grid, interpolation='bilinear',
                ...
                ax[1, 0].imshow(phi_img, interpolation='nearest',
                                aspect='auto', cmap=cm.jet)
                ax[1, 0].set_yticks(np.arange(0, len(args_text) + 1))
                ax[1, 0].set_yticklabels(
                    list(' ' + args_text[::-1]), rotation='vertical', fontsize=8)
                ax[1, 0].grid(False)
                ax[1, 0].set_title('Phi')

                window_img = np.vstack(window_data).T
                ax[1, 1].imshow(window_img, interpolation='nearest',
                                aspect='auto', cmap=cm.jet)
                ax[1, 1].set_yticks(np.arange(0, len(charset)))
                ax[1, 1].set_yticklabels(
                    list(charset), rotation='vertical', fontsize=8)
                ax[1, 1].grid(False)
                ...
                for stroke in split_strokes(cumsum(np.array(coords))):
                    if args.signature:
                        plt.plot(stroke[:, 0], -stroke[:, 1], color='0.0')
                    else:
            ...
            if args.animation:
                fig, ax = plt.subplots(1, 1, frameon=False, figsize=(
                    2 * (maxx - minx + 2) / (maxy - miny + 1), 2))
